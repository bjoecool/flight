...
def request_one_flight_by_url(task_id, url, store_dir="results", proxies=None):
    """
...
#         return
    ...
    while(retry_number < 3):
        try:
            ...
            retry_number = retry_number + 1
            if retry_number >= 3:
                error_logger.error("Error request url : %s , has reached max times[%d]" % (url, retry_number))
                return None
            continue
    ...
    while(retry_number < 3):
        try:
            ...
            retry_number = retry_number + 1
            if retry_number >= 3:
                error_logger.error("Error request json file : %s , has reached max times[%d]" % (json_url, retry_number))
                return None
            continue
        except Exception as err:
